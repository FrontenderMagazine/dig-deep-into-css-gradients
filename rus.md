...
`content-box`), в случаях если для `background-origin` и `background-clip` значение одинаковое,
...
будет отличаться (исключением является значение 100%, которое можно и не указывать
